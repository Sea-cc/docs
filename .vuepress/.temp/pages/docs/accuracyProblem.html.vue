<template><div><h1 id="✍️-js-精度问题" tabindex="-1"><a class="header-anchor" href="#✍️-js-精度问题" aria-hidden="true">#</a> ✍️ JS 精度问题</h1>
<h5 id="前言" tabindex="-1"><a class="header-anchor" href="#前言" aria-hidden="true">#</a> 前言：</h5>
<p>在和后端接口联调时发现一个细节又很基础的问题：后端返回一个树的 id <code v-pre>3068734572459725762</code>，而我在前台接收为 <code v-pre>3068734572459726000</code>😅【很奇怪】</p>
<p>按道理说这 ID 是自动生成的也不可能会改吧，在我百思不得其解时，我的启蒙让我在浏览器控制台输出看一下。三七二十一、0.1+0.2 != 0.3 :</p>
...
<p><strong><a href="https://github.com/sidorares/json-bigint" target="_blank" rel="noopener noreferrer">json-bigint<ExternalLinkIcon/></a> 是一个第三方包，它在把 json 字符串转 json 对象的过程中，自动识别大整数，把大整数转成一个对象来表示，这样就不会产生精度丢失的问题了。</strong></p>
