import { setupDevtoolsPlugin } from './chunk-BXUUIO7M.js';
import {
	computed,
	defineComponent,
	getCurrentInstance,
...
var matchedRouteKey = PolySymbol(
	true ? 'router view location matched' : 'rvlm'
);
...
You can find more information at https://next.router.vuejs.org/docs/migration/#usage-of-history-state.`);
		}
	...
	function addRoute(record, parent, originalRecord) {
		...
		for (const normalizedRecord of normalizedRecords) {
			...
			if (normalizedRecord.path === '*') {
				throw new Error(
					'Catch all routes ("*") must now be defined using a param with a custom regexp.\nSee more at https://next.router.vuejs.org/docs/migration/#removed-star-or-catch-all-routes.'
				);
			}
				...
				if (
					...
					warn(
						`Component "${name}" in record with path "${
							record.path
						}" is not a valid component. Received "${String(
							rawComponent
						)}".`
					);
			...
			api.on.getInspectorState((payload) => {
				if (
					...
					if (route) {
						payload.state = {
							options:
								formatRouteRecordMatcherForStateInspector(
									route
								),
						};
					}
 ...
 when navigating to "${
	...
	function navigate(to, from) {
		...
		return runGuardQueue(guards)
			...
			.catch((err) =>
				isNavigationFailure(err, 8) ? err : Promise.reject(err)
			);
	}
